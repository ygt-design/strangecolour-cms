...
import { buildAreNaChannelWebUrl, createSlashChannelForCurrent } from "../../arena";
...
const Heading = styled.h1`
  font-family: ${FONT_STACK};
  /* Citerne-Bold.woff2 is registered at font-weight 700 in styles.js */
  font-weight: 700;
  font-style: normal;
...
export default function CreateSlashChannel() {
  ...
  const thumbPreviewUrl = useObjectUrl(thumbnailFile);
  ...
  const handleImagesChange = useCallback((event) => {
  ...
  async function handleSubmit(event) {
    ...
    try {
      const result = await createSlashChannelForCurrent({
        title,
        subtitle,
        thumbnailFile,
        imageFiles,
          ...
          <Card>
            ...
            <FormLayout onSubmit={handleSubmit}>
              ...
              <FieldWrap>
                ...
                {previewTitle && (
                  <PreviewLabel>
                    Entry title on Are.na: <strong>{previewTitle}</strong>
                  </PreviewLabel>
                )}
              </FieldWrap>
              ...
              <FieldWrap>
                ...
                />
                <DropZone
                  $isDragging={thumbDrag.isDragging}
                  $hasFiles={!!thumbnailFile}
                  onDragEnter={thumbDrag.onDragEnter}
                  onDragLeave={thumbDrag.onDragLeave}
                  onDragOver={thumbDrag.onDragOver}
                ...
                >
                  {thumbnailFile ? (
                    <DropLabel>
                      ...
                      <DropAccent as="label" htmlFor="thumbnail-file">replace</DropAccent>
                      {" "}or paste to replace
                    </DropLabel>
                  ) : (
                    <DropLabel>
                      Drag, <DropAccent as="label" htmlFor="thumbnail-file">browse</DropAccent>,
                      or paste an image
                    </DropLabel>
                  )}
                </DropZone>
                ...
                {thumbPreviewUrl && (
                  <SinglePreview>
                    <img src={thumbPreviewUrl} alt="Thumbnail preview" />
                    <RemoveButton
                      type="button"
              ...
              <FieldWrap>
                ...
                >
                  <DropLabel>
                    {imageFiles.length > 0
                      ? `${imageFiles.length} extra image${imageFiles.length === 1 ? "" : "s"} for this entry — click or drop to add more`
                      : <>Optional gallery images for the same entry. Drag here or <DropAccent>browse</DropAccent></>}
                  </DropLabel>
                </DropZone>
                <Hint>You can skip this; extra images show in the entry carousel on Current.</Hint>
              ...
              <SidePanel>
                <SidePanelCard>
                  ...
                  <SubmitButton type="submit" $disabled={isSubmitting} style={{ width: "100%" }}>
                    {isSubmitting ? "Submitting..." : "Submit current entry"}
                  </SubmitButton>
                </SidePanelCard>
